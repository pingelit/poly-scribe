...
def test__render_pyproject_toml():
    ...
    assert toml_result["build-system"]["requires"] == ["setuptools"]
    assert toml_result["build-system"]["build-backend"] == "setuptools.build_meta"

    assert toml_result["project"]["name"] == additional_data["package"]
    assert toml_result["project"]["description"] == "Code generated by poly-scribe-code-gen"
    assert toml_result["project"]["version"] == "0.1.0"
    assert toml_result["project"]["authors"] == [
        {"name": additional_data["author_name"], "email": additional_data["author_email"]}
    ]
    assert toml_result["project"]["license"] == additional_data["licence"]
    assert "pydantic" in toml_result["project"]["dependencies"]
    assert "jsonschema" in toml_result["project"]["dependencies"]
    assert "strenum" in toml_result["project"]["dependencies"]
    assert "pydantic-yaml" in toml_result["project"]["dependencies"]
    assert "cbor2" in toml_result["project"]["dependencies"]
